...
End Sub
Sub Calculate_Pool_PV_From_Yield()
    ' INPUT: E12 = BEY Yield
    ' OUTPUT: E13 = Calculated PV, E14 = Pool Price
    ' This does NOT use Goal Seek - just calculates PV directly
    ...
    Dim pool_yield_monthly As Double
    Dim lastRow As Long, i As Long
    Dim period As Double, cashflow As Double
    Dim poolPV As Double
    ...
    If wsPool Is Nothing Then
        MsgBox "Pool CF sheet not found. Please run the CF creation macro first.", vbExclamation
        GoTo ErrorHandler
    End If
    ...
    ' Calculate PV for each period in column P
    For i = 12 To lastRow
        period = 0
        cashflow = 0
        
        If IsNumeric(wsPool.Cells(i, "B").Value) Then
            period = wsPool.Cells(i, "B").Value
        End If
        
        If IsNumeric(wsPool.Cells(i, "O").Value) Then
            cashflow = wsPool.Cells(i, "O").Value
        End If
        
        If period <> 0 Or cashflow <> 0 Then
            wsPool.Cells(i, "P").Value = cashflow / ((1 + pool_yield_monthly) ^ (period / 12))
        Else
            wsPool.Cells(i, "P").Value = ""
        End If
    Next i
    ...
    ' Output to Assumption tab
    wsAssump.Range("E13").Value = poolPV
    wsAssump.Range("E13").NumberFormat = "#,##0"
    
    ' Calculate Pool Price
    If wsPool.Range("C1").Value <> 0 Then
        wsAssump.Range("E14").Value = poolPV / wsPool.Range("C1").Value
        wsAssump.Range("E14").NumberFormat = "0.0000%"
    Else
        wsAssump.Range("E14").Value = ""
    End If
...
ErrorHandler:
    ...
    Else
        MsgBox "PV Calculation Complete!" & vbCrLf & vbCrLf & _
               ...
               "Monthly Yield: " & Format(pool_yield_monthly, "0.0000%") & vbCrLf & _
               "Calculated PV (E13): " & Format(poolPV, "#,##0") & vbCrLf & _
               "Pool Price (E14): " & Format(wsAssump.Range("E14").Value, "0.0000%"), vbInformation
    End If
End Sub
